...
uint8_t midi_qpop_packet(){
	if(q_tail->cin == EMPTY_PACKET_CIN){
		return 0;
	}
	...
	printf(" sending packet {0x%02X, 0x%02X, 0x%02X, 0x%02X, 0x%02X, 0x%02X, 0x%02X, 0x%02X}\n",
			q_tail->cin,
			q_tail->b0,
			q_tail->b1,
			q_tail->b2,
...
int midi_main(){
	...
	midi_qpush_sysex_dump((uint8_t*)settings, sizeof(deadbrain_config));
	for(int i = 0; i < 500; i++){
		midi_qpop_packet();
	}
}
